...
import { fetchNewsList, deleteNews, type NewsItem } from '@/api/news'
...
export default function AdminNewsPage() {
  ...
  const load = useCallback(async (p: number, kw: string) => {
    setLoading(true)
    try {
      const res = await fetchNewsList({ page: p, size: PAGE_SIZE, keyword: kw, type: 2 })
      setItems(res.items)
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(checkedIds.map((id) => deleteNews(id)))
      load(page, keyword)
    } catch (err: unknown) {
      alert(err instanceof Error ? err.message : '삭제에 실패했습니다.')
    }
        ...
        <main className="adm_main">
          <section className="adm_section">
            ...
            <div className="adm_toolbar">
              ...
              <button className="adm_btn_primary" onClick={() => navigate('/admin/news/write')}>
                + 글쓰기
              </button>
            </div>
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr key={item.id}>
                      ...
                      <td>
                        <Link to={`/admin/news/edit/${item.id}`} className="adm_table_link">
                          {item.title}
                        </Link>
                      </td>
