...
import { fetchHealthInfoList, deleteHealthInfo, toggleHealthInfoPin, type HealthInfoItem } from '@/api/board'

const PAGE_SIZE = 15

type HealthInfoSearchParams = { keyword: string; type: number; date_from: string; date_to: string; is_pinned: string }
const defaultHealthInfoParams: HealthInfoSearchParams = { keyword: '', type: 2, date_from: '', date_to: '', is_pinned: '' }

export default function AdminHealthInfoPage() {
  const navigate = useNavigate()
  const [items, setItems]           = useState<HealthInfoItem[]>([])
  const [totalCount, setTotalCount] = useState(0)
  const [totalPages, setTotalPages] = useState(1)
  const [page, setPage]             = useState(1)
  const [searchParams, setSearchParams] = useState<HealthInfoSearchParams>(defaultHealthInfoParams)
  ...
  const load = useCallback(async (p: number, params: HealthInfoSearchParams) => {
    setLoading(true)
    try {
      const res = await fetchHealthInfoList({ page: p, size: PAGE_SIZE, ...params })
      setItems(res.items)
  ...
  const handleReset = () => {
    ...
    setInputIsPinned('')
    setPage(1)
    setSearchParams(defaultHealthInfoParams)
  }
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(checkedIds.map((id) => deleteHealthInfo(id)))
      load(page, searchParams)
    } catch (err: unknown) {
      alert(err instanceof Error ? err.message : '삭제에 실패했습니다.')
    }
          ...
          <section className="adm_section">
            <div className="adm_toolbar">
              ...
              <button
                ...
                onClick={() => navigate('/admin/health-info/write')}
              >
                + 글쓰기
              </button>
            </div>
                      ...
                      <tr key={item.id} style={item.is_pinned ? { background: '#f8f8f8' } : undefined}>
                        ...
                        <td>
                          <button
                            ...
                            onClick={() => navigate(`/admin/health-info/edit/${item.id}`)}
                          >
                        ...
                        <td className="adm_td_center">
                          <div className="adm_action_btns">
                            ...
                            <button
                              ...
                              onClick={() =>
                                navigate(`/admin/health-info/edit/${item.id}`)
                              }
                            >
                              수정
                            </button>
