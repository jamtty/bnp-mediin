...
export default function AdminVoiceDetailPage() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const data = await fetchVoiceDetail(numId)
      setDetail(data)
  ...
  const handleReplySave = async () => {
    ...
    setReplySaving(true)
    try {
      await replyVoice(numId, replyContent)
      alert('답변이 등록되었습니다.')
      load()
    } catch (err: unknown) {
    ...
    return (
      <div className="adm_wrap">
        ...
        <div className="adm_content">
          <AdminHeader pageTitle="고객의소리 상세" />
            ...
            <div className="adm_view_table_wrap">
              <table className="adm_view_table">
                <tbody>
                  <tr>
                    <th>분류</th>
                    <td>{VOICE_CATEGORY_MAP[detail.category] ?? detail.category ?? '-'}</td>
                  </tr>
                  <tr>
                    <th>이름</th>
                    <td>{detail.name}</td>
