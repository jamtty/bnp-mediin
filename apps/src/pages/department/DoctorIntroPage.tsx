...
import { fetchPublicDoctors, DEPT_CODE_MAP, DEPT_GROUPS, type DoctorItem } from '../../api/doctor'
import DoctorScheduleModal from '../../components/DoctorScheduleModal'

const DEPT_FILTER_LIST = [
  { code: 'all', label: '전체' },
  ...Object.values(DEPT_GROUPS)
    .flat()
...
export default function DoctorIntroPage() {
  ...
  const groupedByDept: { code: string; label: string; doctors: DoctorItem[] }[] =
    activeDept === 'all'
      ? Object.values(DEPT_GROUPS)
          .flat()
          .filter((dept, idx, arr) => arr.findIndex((x) => x.code === dept.code) === idx)
          .map(({ code, name }) => ({
            code,
            label: name,
            doctors: allDoctors.filter((d) => getDeptCodes(d).includes(code)),
          }))
  ...
  return (
    ...
    >
      ...
      {scheduleDoctor && (
        <DoctorScheduleModal doctor={scheduleDoctor} onClose={() => setScheduleDoctor(null)} currentDeptName={scheduleDoctor.doc_major ?? ''} />
      )}
    </SubPageLayout>
  )
}
