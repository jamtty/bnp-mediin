...
export default function HomePage() {
    ...
    useEffect(() => {
        ...
        const bmtIdxArr = activeTab === 0 ? [1, 2, 7] : [activeTab]
        ...
        fetch(`/board/board_main_list.json?${params}`)
            ...
            .then(data => {
                if (data.result === 'success' && data.paging.result.length > 0) {
                    ...
                    if (activeTab === 0 || activeTab === 1) setMoreHref('/main/board/1/board_list.do')
                    else if (activeTab === 7) setMoreHref('/main/board/7/board_list.do')
                    else if (activeTab === 2) setMoreHref('/main/board/2/board_list.do')
                }
            })
            .catch(() => {})
    }, [activeTab])
    ...
    <div className='wrap'>
        ...
        <div id="container">
            <div className="main_visual_wrap">
                ...
                <div className="visual_txt_box">
                    ...
                    <div className="txt">
                        <p className="vs_tt"><img src={vsTit} alt="MEDI-IN HOSPITAL" /></p>
                        <p className="vs_txt">메디인 병원,
                            <br />보건복지부 의료기관 <br className="pc_hide" />인증병원
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div className="main_quick_menu">
                    <div className="quick_item cs_type">
                        <a href="#" onClick={(e) => { e.preventDefault(); }}>
                            <dl>
                                ...
                                <dd>
                                    <ul>
                                        <li><strong className="label">·진료예약 콜센터</strong><span>1566-1991</span></li>
                                        <li><strong className="label">·건강증진센터</strong><span>031)570-9026~7</span></li>
                                        <li><strong className="label">·장례식장</strong><span>031)570-9093</span></li>
                                        <li><strong className="label">·응급실</strong><span>031)570-7911</span></li>
                                    </ul>
                                </dd>
                            </dl>
                    ...
                    <div className="quick_item">
                        <a href="/fune/" target="_blank" rel="noreferrer">
                            <dl>
                                <dt>장례식장</dt>
                                <dd>마지막 가시는길 <br />가족의 마음으로 <br />메디인병원이 함께합니다.</dd>
                            </dl>
                    ...
                    <div className="swiper" ref={swiperRef}>
                        <div className="swiper-wrapper">
                            {[
                                ...
                                { href: '/main/content/view/MN02020200.do', img: slide2, name: '인공관절 클리닉' },
                                { href: '/main/content/view/MN02020300.do', img: slide3, name: '척추 클리닉' },
                                { href: '/main/content/view/MN02020400.do', img: slide4, name: '수지접합 클리닉' },
                                ...
                                { href: '/main/content/view/MN02020800.do', img: slide8, name: '중재시술 클리닉' },
                                { href: '/main/content/view/MN02020900.do', img: slide9, name: '뇌신경질환 클리닉' },
                                { href: '/main/content/view/MN02021000.do', img: slide10, name: '요로결석 클리닉' },
                            ].map((item) => (
                                <div key={item.href} className="slide_item swiper-slide">
                                    <a href={item.href}>
                                        <div className="special_img"><img src={item.img} alt={item.name} /></div>
            ...
            <div className="main_news_sec">
                ...
                <div className="news_cont_wrap">
                    <div className="news_top">
                        <button type="button" className={`btn_tab${activeTab === 0 ? ' active_tab' : ''}`} aria-label="메디인 소식 전체" onClick={() => setActiveTab(0)}>전체</button>
                        <button type="button" className={`btn_tab${activeTab === 1 ? ' active_tab' : ''}`} aria-label="메디인 소식 중 공지사항" onClick={() => setActiveTab(1)}>공지사항</button>
                        <button type="button" className={`btn_tab${activeTab === 7 ? ' active_tab' : ''}`} aria-label="메디인 소식 중 건강정보" onClick={() => setActiveTab(7)}>건강정보</button>
                        <button type="button" className={`btn_tab${activeTab === 2 ? ' active_tab' : ''}`} aria-label="메디인 소식 중 보도자료" onClick={() => setActiveTab(2)}>보도자료</button>
                    </div>
                    <a href={moreHref} className="btn_more" title="메디인 소식 더보기">더보기<i className="ico_plus"></i></a>
                            ...
                            <div key={i} className="news_item">
                                {item.bmt_idx === 2
                                    ? <a href={item.bd_field_2} target="_blank" rel="noreferrer">
                                        ...
                                        {item.tFileVo && <div className="bbs_thumb"><img src={`https://medi-in.co.kr${item.tFileVo.file_src}`} alt={item.tFileVo.file_cn} /></div>}
                                    ...
                                    : <a href={`/main/board/${item.bmt_idx}/${item.bd_idx}/board_view.do`}>
                                        <span className={item.bmt_idx === 1 ? 'state_notice' : 'state_news'}>{item.bmt_name}</span>
                                        {item.tFileVo && <div className="bbs_thumb"><img src={`https://medi-in.co.kr${item.tFileVo.file_src}`} alt={item.tFileVo.file_cn} /></div>}
                                        <p className="news_tit">{item.bd_title}</p>
                                        <p className="news_txt">{item.bd_content}</p>
                                        <p className="news_date">{item.inputdate.substring(0, 10)}</p>
            ...
            <div className="main_location_sec">
                <div className="location_info">
                    ...
                    <p className="loca_info_txt">
                        ...
                        정보화 시대와 온라인 진료 시대에 발 맞추어 질 좋은 의료와 친절한 병원으로서<br className="m_hide" />
                        파주 경기북부지역의 건강의 파수꾼이 되고, 최신장비와 최고의 기술로<br className="m_hide" />
                        헌신적인 진료와 정직한 의료서비스를 통해 지역사회에 봉사하며<br className="m_hide" />
        ...
        <div id="side_fix" className={sideFixClosed ? 'close_side' : ''}>
            <button type="button" className="btn_side_toggle" onClick={() => setSideFixClosed(prev => !prev)}>사이드퀵 메뉴 열고 닫기</button>
            <div className="quick_wrap">
                ...
                <div className="quick_cont">
                    <div className="quick_item">
                        <button type="button" className="quick_link" onClick={() => setActivePopup('fast_reserv')}>
